...
BETWEEN        = Regex('[,;]?\s*?(and )?between'                                      , IGNORECASE)
BROADLY        = Regex('[,;]?\s*?(broadly|generally)'                                 , IGNORECASE)
...
EXCEPT         = Regex('[,;]?\s*?except( for)?'                                       , IGNORECASE)
...
RISKING        = Regex(',?\s*?risking'                                                , IGNORECASE)
ROLE           = Regex('(?P<role>agent|service|storage)'                              , IGNORECASE)
TO_FROM        = Regex('[,;]?\s*?(to|from)'                                           , IGNORECASE)
VERIFIED       = Regex('(?P<verified>verified|checked)'                               , IGNORECASE)
WITHIN         = Regex('[,;]?\s*?(and )?within'                                       , IGNORECASE)
WITH_NOTES     = Regex('[,;]?\s*?(with )?not(es?|ing) ?(that)?'                       , IGNORECASE)
...
DESCRIPTION   = QuotedString('"', escQuote='""', multiline=True).setResultsName('description')
NOTES         = QuotedString('"', escQuote='""', multiline=True).setResultsName('notes'      )
